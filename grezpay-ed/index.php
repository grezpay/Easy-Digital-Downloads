<?php
/*
Plugin Name: EDD - grezpay Gateway
Plugin URL: http://easydigitaldownloads.com/extension/grezpay-gateway
Description: A grezpay gateway for Easy Digital Downloads
Version: 1.0
Author: Rohit Kr Singh
Author URI: https://www.grezpay.com/
...
function pw_edd_register_gateway($gateways) {
	$gateways['grezpay_gateway'] = array('admin_label' => 'grezpay Gateway', 'checkout_label' => __('grezpay Gateway', 'pw_edd'));
	return $gateways;	
}
add_filter('edd_payment_gateways', 'pw_edd_register_gateway');

function pw_edd_grezpay_gateway_cc_form() {
	// register the action to remove default CC form
	return;
}
add_action('edd_grezpay_gateway_cc_form', 'pw_edd_grezpay_gateway_cc_form');
...
add_action('edd_gateway_grezpay_gateway', 'pw_edd_process_payment');
...
//Return Response
function edd_listen_for_grezpay_gateway() {

	if ( isset( $_POST['RESPONSE_CODE'] ) && $_POST['CUST_EMAIL'] != "" ) {
            do_action( 'edd_verify_grezpay_gateway' );    
                   
	}
}
add_action( 'init', 'edd_listen_for_grezpay_gateway' );



function edd_process_grezpay_gateway() {
    global $edd_options;
    $payment = edd_insert_payment($payment);
    $order_id=$payment;
          
		  if($_POST['STATUS']!='Captured'){
		            edd_send_back_to_checkout( '?payment-mode=grezpay_gateway' );            
		        }
           
           if($_POST['STATUS']=='Captured')
					{
						$payment_meta   = edd_get_payment_meta( $order_id );
						edd_insert_payment_note( $order_id, sprintf( __( 'Thank you for your order . Your transaction has been successful. grezpay Transaction ID: %s', 'edd' ) , $_POST['STATUS'] ) );
						...
						edd_record_gateway_error( __( 'grezpay Error', 'edd' ), sprintf( __( 'It seems some issue in server to server communication. Kindly connect with administrator.', 'edd' ), '' ), $payment_id );
						...
						wp_redirect( '?page_id=6&payment-mode=grezpay_gateway' );
					}
...
add_action( 'edd_verify_grezpay_gateway', 'edd_process_grezpay_gateway' );
...
function pw_edd_add_settings($settings) {
	...
	$grezpay_gateway_settings = array(
		array(
			'id' => 'grezpay_gateway_settings',
			'name' => '<strong>' . __('grezpay Gateway Settings', 'pw_edd') . '</strong>',
		...
		array(
			...
			'desc' => __('Transaction URL Provided by Grezpay', 'pw_edd'),
			'type' => 'text',
			'size' => 'regular'
		),
		array(
	...
	return array_merge($settings, $grezpay_gateway_settings);	
}
